...
```
clean test "-Dbrowser=$BROWSER" "-DbaseUrl=$BASE_URL" "-Dremote=https://user1:1234@${REMOTE}/wd/hub" -DbrowserSize=$BROWSER_SIZE -DbrowserVersion=$BROWSER_VERSION
```
...
Во вкладке *Дашборды* в <code>Allure TestOps</code> видна статистика количества тестов: сколько из них добавлены и проходятся вручную, сколько автоматизированы. Новые тесты, а так же результаты прогона приходят по интеграции при каждом запуске сборки.
